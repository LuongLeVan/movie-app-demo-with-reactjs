...
const Series = () => {
    const { listFilm, setYearTV, yearTV, setTypeTV, setPageTV, totalPageTV, sencondType, setSencondType } =
        useContext(MovieConntext);
    const [typeMenu, setTypeMenu] = useState([]);

    const fetchListType = async () => {
        const { data } = await axios.get(
            'https://api.themoviedb.org/3/genre/tv/list?api_key=6de13cb06459580e8f7ab054d9dbb28e&language=en-US',
        );
            ...
            <div>
                <div className={cx('wrapper')}>
                    ...
                    <div onClick={handleScroll}>
                        <span className={cx('scroll-button')}>
                            <ArrowUpwardIcon />
                        </span>
                    </div>
                                    ...
                                    <div className={cx('row')}>
                                        <div className={cx('column')}>
                                            {typeMenu.map((item, index) => (
                                                <div
                                                    className={cx('item_select')}
                                                    key={index}
                                                    onClick={() => setTypeTV(item.id)}
                                                >
                                                    {item.name}
                                                </div>
                                            ))}
                    ...
                    <div className="grid wide">
                        ...
                        <div className={cx('wrapper-panigation')}>
                            <PaginationSize
                                Movie={false}
                                setPageTV={setPageTV}
                                totalPageTV={totalPageTV > 500 ? 500 : totalPageTV}
                            />
                        </div>
                    </div>
                </div>
